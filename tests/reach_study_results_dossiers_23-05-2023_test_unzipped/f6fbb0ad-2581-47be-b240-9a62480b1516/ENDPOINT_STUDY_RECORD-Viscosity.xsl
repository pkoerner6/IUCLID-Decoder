...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-Viscosity/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:Viscosity">
        <tr>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:Temp | //xt:Parameter | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '3423'">viscosity</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1309'">OECD Test Guideline 114 (Viscosity of Liquids)</xsl:when>
                    <xsl:when test="./i6:value = '3890'">EPA OPPTS 830.7100 (Viscosity)</xsl:when>
                    <xsl:when test="./i6:value = '2246'">other guideline:</xsl:when>
                    <xsl:when test="./i6:value = '60143'">ASTM D-88 (Standard Test Method for Saybolt Viscosity)</xsl:when>
                    <xsl:when test="./i6:value = '60144'">ASTM D-445 (Standard Test Method for Kinematic Viscosity of Transparent and Opaque Liquids (and Calculation of Dynamic Viscosity)</xsl:when>
                    <xsl:when test="./i6:value = '60145'">ASTM D-562 (Standard Test Method for Consistency of Paints Measuring Krebs Unit (KU) Viscosity Using a Stormer-Type Viscometer)</xsl:when>
                    <xsl:when test="./i6:value = '60146'">ASTM D-1200 (Standard Test Method for Viscosity by Ford Viscosity Cup)</xsl:when>
                    <xsl:when test="./i6:value = '60147'">ASTM D-2196 (Standard Test Methods for Rheological Properties of Non-Newtonian Materials by Rotational (Brookfield type) Viscometer)</xsl:when>
                    <xsl:when test="./i6:value = '60148'">ASTM D-2983 (Standard Test Method for Low-Temperature Viscosity of Lubricants Measured by Brookfield Viscometer)</xsl:when>
                    <xsl:when test="./i6:value = '60149'">ASTM D-4486 (Standard Test Method for Kinematic Viscosity of Volatile and Reactive Liquids)</xsl:when>
                    <xsl:when test="./i6:value = '60150'">BS 188 (Methods for determination of the viscosity of liquids)</xsl:when>
                    <xsl:when test="./i6:value = '60151'">CIPAC MT 3 (Specific gravity, density and weight per millilitre)</xsl:when>
                    <xsl:when test="./i6:value = '60152'">CIPAC MT 192 (Viscosity of liquids by rotational viscometry)</xsl:when>
                    <xsl:when test="./i6:value = '60153'">DIN 51366 (Testing of mineral oil hydrocarbons - Measurement of kinematic viscosity by means of the Cannon-Fenske viscometer for opaque liquids)</xsl:when>
                    <xsl:when test="./i6:value = '60154'">DIN 51377 (Testing of lubricants - Determination of the apparent viscosity of motor oils at low temperature from -5 °C to -35 °C - Using the cold-cranking simulator)</xsl:when>
                    <xsl:when test="./i6:value = '60155'">DIN 51562 Part 1 (Viscometry - Measurement of kinematic viscosity by means of the Ubbelohde viscometer - Part 1: Viscometer specification and measurement procedure)</xsl:when>
                    <xsl:when test="./i6:value = '60156'">DIN 52007-2 (Bitumen and bituminous binders - Determination of viscosity - Part 2: Measurement by drawn-sphere viscometer)</xsl:when>
                    <xsl:when test="./i6:value = '60157'">DIN 53015 (Viscometry - Measurement of viscosity by means of the rolling ball viscometer by Höppler)</xsl:when>
                    <xsl:when test="./i6:value = '60158'">DIN 53019 Part 1 (Viscometry - Measurement of viscosities and flow curves by means of rotational viscometers - Part 1: Principles and measuring geometry)</xsl:when>
                    <xsl:when test="./i6:value = '60159'">DIN 53177 (Binders for paints and varnishes - Measurement of the dynamic viscosity of liquid resins; Resin solutions and oils by the capillary viscosimeter of isocelses type according to Ubbelohde)</xsl:when>
                    <xsl:when test="./i6:value = '60160'">DIN 53214 (Testing of paints and varnishes - Determination of rheograms and viscosities by rotational viscometers)</xsl:when>
                    <xsl:when test="./i6:value = '60161'">DIN EN ISO 12058-1 Plastics - Determination of viscosity using a falling-ball viscometer - Part 1: Inclined-tube method</xsl:when>
                    <xsl:when test="./i6:value = '60162'">ISO 2431 (Paints and varnishes - Determination of flow time by use of flow cups)</xsl:when>
                    <xsl:when test="./i6:value = '60163'">ISO 3104 (Petroleum products - Transparent and opaque liquids - Determination of kinematic viscosity and calculation of dynamic viscosity)</xsl:when>
                    <xsl:when test="./i6:value = '60164'">ISO 3105 (Glass capillary kinematic viscometers - Specifications and operating instructions)</xsl:when>
                    <xsl:when test="./i6:value = '60165'">ISO 3219 (Plastics; polymers/resins in the liquid state or as emulsions or dispersions; determination of viscosity using a rotational viscometer with defined shear rate)</xsl:when>
                    <xsl:when test="./i6:value = '60166'">NF T60-100 (Petroleum products. Transparent and opaque liquids. Determination of kinematic viscosity and calculation of dynamic viscosity; identical to ISO 3104)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60651'">capillary viscometer  (static)</xsl:when>
                    <xsl:when test="./i6:value = '1497'">rotational viscometer (dynamic)</xsl:when>
                    <xsl:when test="./i6:value = '775'">forced ball viscometer (dynamic)</xsl:when>
                    <xsl:when test="./i6:value = '60655'">rolling ball viscometer (dynamic)</xsl:when>
                    <xsl:when test="./i6:value = '60656'">drawing ball viscometer (dynamic)</xsl:when>
                    <xsl:when test="./i6:value = '60168'">flow cup (static)</xsl:when>
                    <xsl:when test="./i6:value = '60666'">20°C</xsl:when>
                    <xsl:when test="./i6:value = '60667'">40°C</xsl:when>
                    <xsl:when test="./i6:value = '60668'">dynamic viscosity (in mPa s)</xsl:when>
                    <xsl:when test="./i6:value = '60669'">kinematic viscosity (in mm²/s)</xsl:when>
                    <xsl:when test="./i6:value = '60670'">flow time (in s) - ASTM Cup 1</xsl:when>
                    <xsl:when test="./i6:value = '60671'">flow time (in s) - ASTM Cup 2</xsl:when>
                    <xsl:when test="./i6:value = '60672'">flow time (in s) - ASTM Cup 3</xsl:when>
                    <xsl:when test="./i6:value = '60673'">flow time (in s) - ASTM Cup 4</xsl:when>
                    <xsl:when test="./i6:value = '60674'">flow time (in s) - 3 mm ISO cup</xsl:when>
                    <xsl:when test="./i6:value = '60675'">flow time (in s) - 4 mm ISO cup</xsl:when>
                    <xsl:when test="./i6:value = '60676'">flow time (in s) - 5 mm ISO cup</xsl:when>
                    <xsl:when test="./i6:value = '60677'">flow time (in s) - 6 mm ISO cup</xsl:when>
                    <xsl:when test="./i6:value = '60678'">flow time (in s) - other:</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:Viscosity | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'Viscosity'">Viscosity</xsl:when>
